...
## 19:04 UTC — Tooling & capture
- Added `scripts/ishtar_telos_capture.py` to snapshot every TELOS markdown file (headings + first lines) and append a timestamped summary to `memory/telos-snapshot.md`. This script is now called by the daily `pai_repo_scan.sh` cron run so TELOS statements are captured automatically. [[Source: scripts/ishtar_telos_capture.py]]
- Added `scripts/ishtar_telos_update_helper.py` to validate TELOS update requests, log them to `memory/ishtar-telos-update.log`, and surface the exact `bun ~/.claude/commands/update-telos.ts` command so when you ask me to record a change I can follow the documented workflow without guessing. [[Source: scripts/ishtar_telos_update_helper.py]]
